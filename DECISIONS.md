...
`persist` (`cell=name persist`) is accepted syntactically
(`parseSqlCellInfo`) but not implemented, and this is a real port
decision, not a missed one. On dewstack, `persist` means: on Run, save
the textarea's current text to `localStorage`; on page load, silently
overwrite the textarea with whatever was saved there, before a reader
sees the fence's own authored text at all. That's safe on dewstack
because the textarea's content is *never* the source of truth — it's
generated markup, thrown away and regenerated by the next build. On
dewnote, a fence's live editor content genuinely is the document's own
text: `blockTexts()` reads it back out on every commit (this file's own
architecture, `app.ts`'s header comment). Porting `persist` unchanged
would mean a restored session silently rewrites the saved markdown file
the next time that block commits — a real correctness hazard dewstack
never has, not a cosmetic gap. Solving it properly needs an actual
answer to "what does a restored session mean for an editable document"
(a separate localStorage-backed overlay that never flows into
`blockTexts()`? a document-scoped key instead of dewstack's page-name-only
one, given dewnote has no separate-tutorial-pages concept to key off of
either?) before porting anything, not a quick copy of dewstack's
mechanism.

...
describe("isCourseFile", () => {
  test("a course file directly inside current courses/ or legacy modules/", () => {
    ...
    expect(isCourseFile("modules/web-authoring.yaml")).toBe(true);
    expect(isCourseFile("some/checkout/modules/web-authoring.yaml")).toBe(true);
  });

  test("index.yaml and redirects.yaml are not courses", () => {
    expect(isCourseFile("modules/index.yaml")).toBe(false);
    expect(isCourseFile("modules/redirects.yaml")).toBe(false);
    expect(isCourseFile("courses/index.yaml")).toBe(false);
    expect(isCourseFile("courses/redirects.yaml")).toBe(false);
  });

  test("anything else", () => {
    expect(isCourseFile("modules/web-authoring.md")).toBe(false);
    expect(isCourseFile("tutorials/first-steps/first-steps.md")).toBe(false);
    // A yaml file one level deeper isn't a course file.
    expect(isCourseFile("modules/old/web-authoring.yaml")).toBe(false);
    expect(isCourseFile("courses/old/web-authoring.yaml")).toBe(false);
  });

  test("a current Dewlab course descriptor parses as a user-facing course", () => {
    const course = parseCourseFile("courses/computational-methods.yaml", COURSE)!;
    expect(course.id).toBe("computational-methods");
...
describe("parseCourseFile", () => {
  test("reads the course, its series and their tutorials in order", () => {
    const course = parseCourseFile("modules/programming-design-principles.yaml", COURSE)!;
  ...
  test("each series' range covers exactly its own item lines", () => {
    const course = parseCourseFile("modules/x.yaml", COURSE)!;
  ...
  test("the indent is the one the items actually carry", () => {
    ...
    // would fail here.
    const wide = "id: w\ntitle: W\ncontents:\n  - title: S\n    tutorials:\n        - a\n        - b\n";
    expect(parseCourseFile("courses/w.yaml", wide)!.contents[0]!.indent).toBe("        ");
  });
...
- title: Something
  ...
  - one
`;
    const course = parseCourseFile("modules/x.yaml", source)!;
...
`;
  ...
  test("null for a file that isn't a course, or one it can't read", () => {
    expect(parseCourseFile("modules/index.yaml", "order:\n- one\n")).toBeNull();
    expect(parseCourseFile("modules/x.yaml", "title: [\n")).toBeNull();
    // No title is the one thing dewlab's read_module fails outright on.
    expect(parseCourseFile("modules/x.yaml", "contents: []\n")).toBeNull();
  });
  ...
  test("a course with a title but no contents key at all is read, not refused", () => {
    ...
    const course = parseCourseFile("modules/x.yaml", "title: A course\n");
...
// discipline as the corpus tests in checks.test.ts, and the reason this course was
...
describe("scanContentsBlock, through parseCourseFile", () => {
  ...
  test("the block ends where the next top-level key begins, not at the end of the file", () => {
    ...
    const course = parseCourseFile("modules/oop.yaml", withMixed)!;
  ...
  test("a course whose entries are indented keeps that indent", () => {
    ...
    const course = parseCourseFile("modules/a.yaml", indented)!;
  ...
  test("extra spaces after the dash widen the inner indent, because YAML says they do", () => {
    ...
    const course = parseCourseFile("modules/a.yaml", wide)!;
    expect(course.entryIndent).toBe("");
    expect(course.innerIndent).toBe("    ");
  });
  ...
  test("a course with no series yet offers an empty range where the first one goes", () => {
    ...
    const course = parseCourseFile("modules/a.yaml", empty)!;
    expect(course.contents).toEqual([]);
  ...
  test("a flow contents list offers no range to append to", () => {
    ...
    const course = parseCourseFile("modules/a.yaml", flow)!;
    expect(course.contentsRange).toBeNull();
  });
});
